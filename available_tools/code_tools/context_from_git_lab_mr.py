...
def get_mr_tool_templates() -> List[ToolTemplate]:
    return [
        ToolTemplate(
            name="[gitlab_mr] Context from GitLab MR",
            args={
                ARG_EXTRACT_MODE: EXTRACT_MODE_GIT_MR,
                ARG_PATH_LONG: "~/core_repos/intellian_pkg",
                ARG_GITLAB_MR_URL_LONG: "https://gitlab.com/intellian_adc/prototyping/insensesdk/-/merge_requests/164",
            }
        ),
    ]
...
def main_git_mr(args: argparse.Namespace) -> None:
    ...
    repo_local_path = get_arg_value(args, ARG_PATH_LONG)
    mr_url = get_arg_value(args, ARG_GITLAB_MR_URL_LONG)
    output_dir = get_arg_value(args, ARG_OUTPUT_DIR_LONG)
    no_open_explorer = get_arg_value(args, ARG_NO_OPEN_EXPLORER)
    max_folders = get_arg_value(args, ARG_MAX_FOLDERS)

    if not repo_local_path or not mr_url:
        LOG(f"Error: --path and --mr_url arguments are required for '{EXTRACT_MODE_GIT_MR}' mode.", file=sys.stderr)
        sys.exit(1)
