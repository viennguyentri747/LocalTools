...
def _prompt_non_empty(prompt_text: str, default_value: Optional[str] = None) -> str:
    while True:
        shown_default = f" [{default_value}]" if default_value else ""
        value = input(f"{prompt_text}{shown_default}: ").strip()
        final_value = value or (default_value or "")
        if final_value:
            return final_value
...
def _build_no_setup_post_copy_cmd(original_md5: str, remote_dir: str, remote_name: str) -> str:
    remote_abs_path = f"{remote_dir.rstrip('/')}/{remote_name}"
    return (
        f"actual_md5=$(md5sum {shlex.quote(remote_abs_path)} | cut -d\" \" -f1) && "
        f"echo \"original md5sum: {original_md5}\"; echo \"actual md5sum: $actual_md5\"; "
        f"if [ {shlex.quote(original_md5)} = \"$actual_md5\" ]; then "
        f"echo \"MD5 match: {remote_abs_path}\"; else echo \"MD5 MISMATCH!\"; fi"
    )


def _write_exec_output(exec_output_path: Optional[Path], command: str) -> None:
    if not exec_output_path:
        return
    exec_output_path.parent.mkdir(parents=True, exist_ok=True)
    exec_output_path.write_text(command + "\n")
    LOG(f"{LOG_PREFIX_MSG_INFO} Saved UT-side command to '{exec_output_path}'.")
...
def main() -> None:
    ...
    parser.add_argument(ARG_EXEC_OUTPUT_PATH, default=EMPTY_STR_VALUE, help="Optional local file to save the UT-side command.")
    parser.add_argument(ARG_REMOTE_DIR, default=DEFAULT_REMOTE_DIR, help=f"Remote ACU directory. Defaults to {DEFAULT_REMOTE_DIR}.")
    ...
    add_arg_bool(parser, ARG_PROMPT_BEFORE_EXECUTE, default=True, help_text="Prompt before executing the IESA install command")
    ...
    exec_output_path_str = get_arg_value(args, ARG_EXEC_OUTPUT_PATH)
    exec_output_path = Path(exec_output_path_str).expanduser().resolve() if exec_output_path_str else None
    ...
    copy_to_remote_via_jump_host(local_path=local_file, remote_host_ip=remote_host_ip, remote_dest_path=f"{remote_dir}/{dest_name}",
                                 jump_host_ip=target_ip, remote_user=remote_user, recursive=False)
    ...
    if mode == MODE_BINARY:
        ...
        ut_command = _build_no_setup_post_copy_cmd(original_md5=original_md5, remote_dir=remote_dir, remote_name=dest_name)
        LOG(f"File copied. Run on target UT {target_ip}:")

    print(ut_command)
    _write_exec_output(exec_output_path, ut_command)
