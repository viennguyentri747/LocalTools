...
def get_tool_templates() -> List[ToolTemplate]:
    return [
        ToolTemplate(name="Tail GNSS log", extra_description="Direct tail on an SSM host",
                     args={ARG_HOST_IP: f"{SSM_NORMAL_IP_PREFIX}.57", ARG_REMOTE_PATH: DEFAULT_GNSS_LOG_PATH}),
        ToolTemplate(name="Tail INS monitor log", extra_description="Tail ACU log through an SSM jump host",
                     args={ARG_HOST_IP: ACU_IP, ARG_JUMP_HOST_IP: f"{SSM_NORMAL_IP_PREFIX}.57", ARG_REMOTE_PATH: DEFAULT_INS_MONITOR_LOG_PATH}),
    ]
...
def get_live_remote_log(host_ip: str, remote_log_path: str, user: str = SSM_USER, password: Optional[str] = None, jump_host_ip: Optional[str] = None,
                        jump_user: Optional[str] = None, jump_password: Optional[str] = None, timeout: int = 5, read_timeout: int = 30, tail_lines: int = 0) -> None:
...
def main() -> int:
    ...
    try:
        get_live_remote_log(host_ip=args.host_ip, remote_log_path=args.remote_path, user=args.user, jump_host_ip=args.jump_host_ip, jump_user=args.jump_user,
                            timeout=args.timeout, read_timeout=args.read_timeout, tail_lines=args.tail_lines)
