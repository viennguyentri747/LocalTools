...
from available_tools.test_tools.test_ut_log.test_kim_stats.common import DEFAULT_BASELINE_MAX_DEVIATION, DEFAULT_IMX_REV_STR, DEFAULT_MIN_BASELINE_SINR, ImxSpecs, check_baseline_spread, check_deviation_from_baseline, compact_issues_by_prefix, find_baseline_rows, get_baseline_stats_by_column, get_imx_specs, log_baseline_summary, log_imx_spec, require_columns, SUPPORTED_IMX_REV_STRS
...
RPY_COLUMNS: List[str] = [LAST_ROLL_P_COLUMN, LAST_PITCH_P_COLUMN, LAST_YAW_P_COLUMN]
...
ARG_MAX_BASELINE_RPY_DEVIATION = f"{ARGUMENT_LONG_PREFIX}max_baseline_rpy_deviation"
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(ARG_MAX_BASELINE_RPY_DEVIATION, type=float, default=DEFAULT_BASELINE_MAX_DEVIATION, help=f"Max allowed baseline max-min spread for each RPY column (default: {DEFAULT_BASELINE_MAX_DEVIATION}).")
...
def _write_metadata_file(output_path: Path, input_paths: Sequence[Path], time_window: Optional[float], min_baseline_sinr: float,
                         max_deviation_by_column: Dict[str, float], max_baseline_deviation: float, imx_spec: ImxSpecs, issues: Sequence[str]) -> Path:
    ...
    metadata = {
        ...
        "arguments": {
            ...
            "max_baseline_rpy_deviation": max_baseline_deviation,
        },
...
def check_rpy_file(file_data, min_baseline_sinr: float, max_deviation_by_column: Dict[str, float], max_baseline_deviation: float) -> List[str]:
    ...
    baseline_stats = get_baseline_stats_by_column(file_data, baseline_rows, RPY_COLUMNS)
    log_baseline_summary(file_data, baseline_rows, baseline_stats)
    baseline_limit_by_column = {column: max_baseline_deviation for column in RPY_COLUMNS}
    issues.extend(check_baseline_spread(file_data, baseline_stats, baseline_limit_by_column))
    issues.extend(check_deviation_from_baseline(file_data, baseline_stats, max_deviation_by_column, skip_row_positions=set(baseline_rows.row_positions)))
    return issues
...
def run_kim_rpy_stationary_test(input_paths: Sequence[Path], output_path: Path, time_window: Optional[float], min_baseline_sinr: float,
                                max_deviation_by_column: Dict[str, float], max_baseline_deviation: float, imx_spec: ImxSpecs) -> None:
    ...
    LOG(f"{LOG_PREFIX_MSG_INFO} RPY max deviation thresholds: {max_deviation_by_column}")
    ...
    for file_data in processed_data:
        issues.extend(check_rpy_file(file_data, min_baseline_sinr, max_deviation_by_column, max_baseline_deviation))
    metadata_path = _write_metadata_file(output_path, input_paths, time_window, min_baseline_sinr, max_deviation_by_column, max_baseline_deviation, imx_spec, issues)
...
class KimRpyStationaryPlogTest(TestLogInterface):
    ...
    def run_test(cls, log_paths_by_type: Dict[EUtLogType, List[Path]]) -> None:
        ...
        run_kim_rpy_stationary_test(plog_paths, Path(DEFAULT_OUTPUT_PATH), None, DEFAULT_MIN_BASELINE_SINR, { LAST_ROLL_P_COLUMN: imx_spec.get_roll_max_deviation(), LAST_PITCH_P_COLUMN: imx_spec.get_pitch_max_deviation(), LAST_YAW_P_COLUMN: imx_spec.get_yaw_max_deviation(), }, DEFAULT_BASELINE_MAX_DEVIATION, imx_spec)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    max_deviation_by_column = {
        LAST_ROLL_P_COLUMN: float(roll_deviation_raw) if roll_deviation_raw is not None else imx_spec.get_roll_max_deviation(),
        LAST_PITCH_P_COLUMN: float(pitch_deviation_raw) if pitch_deviation_raw is not None else imx_spec.get_pitch_max_deviation(),
        LAST_YAW_P_COLUMN: float(yaw_deviation_raw) if yaw_deviation_raw is not None else imx_spec.get_yaw_max_deviation(),
    }
    ...
    run_kim_rpy_stationary_test(
        ...
        max_deviation_by_column=max_deviation_by_column,
        max_baseline_deviation=float(get_arg_value(args, ARG_MAX_BASELINE_RPY_DEVIATION)),
        imx_spec=imx_spec,
    )
