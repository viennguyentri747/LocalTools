...
def build_reboot_sequence_command(config: RebootSequenceConfig) -> str:
    ...
    command = (
        ...
        f'prev_req="" && prev_resp="" && dup_count=0 && CURL_RESPONSE="" && '
        # function to curl and log if not duplicate, else increment dup_count
        f'curl_with_log() {{ local url="$1"; local curr_req="GET $url"; CURL_RESPONSE=$(curl -fsS --max-time {curl_timeout_secs} "$url" 2>&1); exit_code=$?; local curr_resp="$CURL_RESPONSE"; if [ "$curr_req" = "$prev_req" ] && [ "$curr_resp" = "$prev_resp" ]; then dup_count=$((dup_count + 1)); else [ $dup_count -gt 0 ] && printf "\\n" >&2; dup_count=0; log "REQUEST: $curr_req"; log "RESPONSE: $curr_resp"; prev_req="$curr_req"; prev_resp="$curr_resp"; fi; return $exit_code; }} && '
        ...
        f'curl_with_log "$BASE_URL/api/system/reboot"; REBOOT_RESPONSE="$CURL_RESPONSE" && '
        ...
        f'if [ $dup_count -gt 0 ]; then log_sameline "[Duplicate, count: $dup_count] Waiting for SSM to respond (elapsed=${{elapsed}}s)"; else log "Waiting for SSM to respond (elapsed=${{elapsed}}s)"; fi; '
        f'sleep "$REQ_INTERVAL"; done && '
        f'ssm_start=$(date +%s) && server_up_time=$(( ssm_start - reboot_start )) && log "SSM up after $server_up_time sec, checking GPX fix status..." && '
        f'until curl_with_log "$BASE_URL/api/gnss/gnssstats" && GPS_DATA="$CURL_RESPONSE" && [ -n "$GPS_DATA" ] && '
        ...
        # CHANGED: Added if/else to combine log messages
        f'if [ $dup_count -gt 0 ]; then log_sameline "[Duplicate, count: $dup_count] Waiting for GPS fix (elapsed=${{elapsed}}s)"; else log "Waiting for GPS fix (elapsed=${{elapsed}}s)"; fi; '
        
        f'sleep "$REQ_INTERVAL"; done && fix_time=$(( $(date +%s) - reboot_start )) && '
        f'log "GPS Fix status ok after $fix_time sec, waiting for CONNECTED status with timeout = $APN_TIMEOUT..." && '
        f'until curl_with_log "$BASE_URL/api/cnx/connection_status" && CNX_STATUS="$CURL_RESPONSE" && echo "$CNX_STATUS" | grep -q \'"connection_status":"CONNECTED"\'; do elapsed=$(( $(date +%s) - ssm_start )); '
        f'[ $elapsed -ge "$APN_TIMEOUT" ] && log "Timed out waiting for CONNECTED!" && exit 0; '
        
        # CHANGED: Added if/else to combine log messages
        f'if [ $dup_count -gt 0 ]; then log_sameline "[Duplicate, count: $dup_count] Waiting for CONNECTED (elapsed=${{elapsed}}s)"; else log "Waiting for CONNECTED (elapsed=${{elapsed}}s)"; fi; '
        ...
        f'log "======================================" && log "CYCLE RESULTS ON {config.ssm_ip}" && log "======================================" && '
        ...
        f'log "Iteration $i:" && '
        f'log "  Total: $tot sec" && log "  SSM up: $sup sec" && log "  GPS 3D fix: $gfix sec" && log "  Connected: $conn sec" && '
        f'log "--------------------------------------" && '
        f'i=$((i + 1)); done && '
        # Summary analysis after each iteration
        f'log "======================================" && log "SUMMARY ANALYSIS ($iteration of $TOTAL_ITERATIONS iterations)" && log "======================================" && '
        ...
        f'calc_stats "$total_times" "Total Time" && calc_stats "$server_up_times" "SSM Up Time" && calc_stats "$gps_fix_times" "GPS 3D Fix Time" && calc_stats "$connect_times" "Connected Time" && '
        f'log "======================================"; done && '
