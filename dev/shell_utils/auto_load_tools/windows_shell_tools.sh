if ! command -v to_windows_path >/dev/null 2>&1; then
    log_err "Required helper 'to_windows_path' is missing. Load common_utils.sh before windows_shell_tools.sh."
fi
...
open_wsl_config() {
    ...
    log "Opening WSL config file: $wsl_config_path"
    vi "$wsl_config_path"
}
...
sync_ssh_keys_to_windows() {
  ...
  log "Syncing SSH keys to: $win_ssh"
...
rm_known_hosts_windows() {
    ...
    if [ -f "$win_known_hosts" ]; then
        log "Removing Windows known_hosts file: $win_known_hosts"
        rm -v "$win_known_hosts"
        log "Removal complete"
    else
        log "No Windows known_hosts file found at: $win_known_hosts"
    fi
}
