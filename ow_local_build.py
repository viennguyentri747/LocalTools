...
def init_and_sync(branch: str) -> None:
    run(
        f"repo init https://gitlab.com/intellian_adc/oneweb_project_sw_tools "
        f"-b {branch} -m {MANIFEST_RELATIVE_PATH}",
        cwd=BUILD_FOLDER,
    )
...
def run_build() -> None:
    run(
        f"docker run -it --rm -v {OW_ROOT}:{OW_ROOT} -w {OW_ROOT} "
