...
fdef() {
    $_DT_GREP_TOOL --display-name "Search C/C++ Definitions" --search-mode fzf-symbols --case-sensitive False --ordered-pattern-keys c-class-struct-definition c-typedef-definition c-function-definition c-enum-definition c-enum-value-definition c-macro-definition c-variable-definition --file-exts $_DT_C_EXTS ${1:+--initial-query "$*"}
    #--path $_DT_CORE_REPOS_PATH
}

fdecl() {
    $_DT_GREP_TOOL --display-name "Search C/C++ Declarations" --search-mode fzf-symbols --case-sensitive False --ordered-pattern-keys c-function-declaration --file-exts $_DT_C_EXTS ${1:+--initial-query "$*"}
}

fsymbol() {
    $_DT_GREP_TOOL --display-name "Search C/C++ Symbols" --search-mode fzf-symbols --case-sensitive False --ordered-pattern-keys c-symbol-usage --file-exts $_DT_C_EXTS ${1:+--initial-query "$*"}
}

ffile() {
    $_DT_GREP_TOOL --display-name "Search Files by Name" --search-mode fzf-files ${1:+--initial-query "$*"}
}

ftext() {
    $_DT_GREP_TOOL --display-name "Search for Text (grep)" --search-mode fzf-text --case-sensitive False --file-exts $_DT_C_EXTS ${1:+--initial-query "$*"}
}