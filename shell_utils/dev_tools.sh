...
fdef() {
    $_DT_GREP_TOOL --display-name "Search C/C++ Definitions" --search-mode fzf-symbols --case-sensitive False --pattern-keys c-function-definition c-variable-definition c-class-struct-definition c-macro-definition c-typedef-definition c-enum-definition c-enum-value-definition --file-exts $_DT_C_EXTS --path $_DT_CORE_REPOS_PATH ${1:+--initial-query "$*"}
}

fsymbol() {
    $_DT_GREP_TOOL --display-name "Search C/C++ Symbols" --search-mode fzf-symbols --case-sensitive False --pattern-keys c-function-call c-symbol-usage --file-exts $_DT_C_EXTS --path $_DT_CORE_REPOS_PATH ${1:+--initial-query "$*"}
}
