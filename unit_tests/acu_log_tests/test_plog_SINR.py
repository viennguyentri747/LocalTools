...
ACU_PLOG_PATH = get_temp_path(ETargetPlatform.WINDOWS) / "acu_plogs/"
EXTRA_DAYS_BEFORE_TODAY = 0
LOG_HOUR_CAPTURE = 0.5  # 0.01 hour = 36 secs
TARGET_COLUMNS = [TIME_COLUMN, LAST_AVG_SINR_COLUMN, LAST_RTK_COMPASS_STATUS_COLUMN]
...
def main():
    ...
    if USE_DUMMY_DATA:
        # Create dummy data
        ut_ip1 = f"{SSM_BASE_IP_PREFIX}.101.97"
        valid_fetch_infos: AcuLogInfo = [
            AcuLogInfo(
                ip=ut_ip1,
                log_paths=[f"{LOCAL_TOOL_REPO_PATH}/temp/acu_plogs/192.168.101.97/P_20250929_000000.txt"],
                is_valid=True
            )
        ]
    else:
        ...
        valid_fetch_infos = batch_fetch_acu_logs_for_days(
            list_ips=[ut_ip1], extra_days_before_today=EXTRA_DAYS_BEFORE_TODAY, log_types=["P"], parent_path=ACU_PLOG_PATH)
    ...
    for ip, ip_log_files in [(fetch_info.ut_ip, fetch_info.log_paths) for fetch_info in valid_fetch_infos]:
        for log_file in ip_log_files:
            ...
            output_path = f"{get_temp_path(ETargetPlatform.WINDOWS)}/PlogTest_output.txt"
            write_to_file(output_path, table_str,
                          mode=WriteMode.OVERWRITE if counter == 0 else WriteMode.APPEND)
            # print(table_str)
            counter += 1

            graph_output_path = f"{get_temp_path(ETargetPlatform.WINDOWS)}/PlogTest_graph_{ip}_{counter}.png"
